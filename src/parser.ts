...
export function parseLine(raw: string, line = 1): ParsedLine {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    else if (commentDepth === 0 && text[i] === ';') break;
  ...
  if (star >= 0) {
    ...
    if (!tail || Number(tail[1]) > 255) return fail('Malformed checksum.');
    let checksum = 0;
  ...
  for (const char of text) {
    ...
    if (char === '(') {
      ...
      if (!inComment) return fail('Unmatched closing comment.');
      inComment = false;
      cleaned += ' ';
    } else if (!inComment) cleaned += char;
  }
  if (inComment) return fail('Unclosed parenthesized comment.');
  cleaned = cleaned.trim();
  if (!cleaned || cleaned === '%') return { command: null, diagnostics };
  ...
  while (rest) {
    ...
    params[letter] = value;
    if (match) rest = rest.slice(match[0].length).trimStart();
  }
