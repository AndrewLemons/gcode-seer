...
export class GcodeAnalyzer {
  ...
  addLine(raw: string | null): void {
    if (this.finished) throw new Error('This analyzer has already finished.');
    ...
    if (/[\r\n]/.test(raw))
      throw new TypeError(
        'addLine expects one line without a line terminator.',
      );
...
class LineFramer {
  ...
  push(chunk: string): void {
    ...
    for (let i = start; i < chunk.length; i++) {
      ...
      if (char === 13) {
        ...
        else if (i === chunk.length - 1) this.afterCR = true;
      }
      start = i + 1;
    }
  ...
  finish(): void {
    if (this.length || this.oversized) this.flush();
  }
  private append(part: string): void {
    if (!part || this.oversized) return;
...
export function analyze(
  ...
  const framer = new LineFramer(
    (line) => analyzer.addLine(line),
    options.maxLineLength ?? 65536,
  );
  framer.push(text);
  framer.finish();
...
export async function analyzeStream(
  ...
  for await (const chunk of chunks) {
    ...
    else throw new TypeError('Expected text or Uint8Array chunks.');
  }
  framer.push(decoder.decode());
  framer.finish();
  return analyzer.finish();
