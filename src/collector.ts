...
const merge = (existing: Box | null, addition: Box): Box => {
  if (!existing) return { min: { ...addition.min }, max: { ...addition.max } };
...
export class Collector {
  ...
  diagnostic(diagnostic: Diagnostic): void {
    ...
    if (diagnostic.category === 'coverage' && diagnostic.severity !== 'info') {
      r.complete = false;
      if (
        ['UNSUPPORTED_COMMAND', 'UNSUPPORTED_PARAMETER'].includes(
          diagnostic.code,
        )
      )
  ...
  event(event: AnalysisEvent): void {
    ...
    const r = this.report;
    if (event.type === 'move') this.move(event);
    else if (event.type === 'dwell') r.nominalDuration += event.seconds;
    else if (event.type === 'tool') r.toolChanges++;
    else {
    ...
    for (const rule of this.options.rules ?? [])
      for (const diagnostic of rule.onEvent(event)) this.diagnostic(diagnostic);
  }
  ...
  private move(move: MoveEvent): void {
      ...
      const leaves = (limit: Box): boolean =>
        (['x', 'y', 'z'] as const).some((axis) =>
          [move.start[axis], move.end[axis]].some(
            (value) =>
              value !== null &&
              (value < limit.min[axis] - 1e-7 ||
                value > limit.max[axis] + 1e-7),
          ),
        );
      for (const [limit, code] of [
        ...
        if (limit && leaves(limit))
          this.violation(
            code,
            move.line,
            'A known axis coordinate leaves the configured bounds.',
          );
    }
    if (bounds) {
      r.bounds = merge(r.bounds, bounds);
      if (extruding) r.extrusionBounds = merge(r.extrusionBounds, bounds);
      ...
      if (
        toolProfile?.travelBounds &&
        !containsBox(toolProfile.travelBounds, bounds)
      )
        this.violation(
          'TOOL_TRAVEL_BOUNDS',
          move.line,
      ...
      if (
        extruding &&
        toolProfile?.printBounds &&
        !containsBox(toolProfile.printBounds, bounds)
      )
        this.violation(
          'TOOL_PRINT_BOUNDS',
          move.line,
    ...
    const checkPath =
      move.path ??
      (bounds
        ? { kind: 'line' as const, start: bounds.min, end: bounds.max }
        : null);
    for (const zone of p?.exclusions ?? [])
      if (
        ...
        this.violation(
          'EXCLUSION',
          move.line,
          `Movement intersects exclusion "${zone.id}".`,
        );
    if (move.axisSpeeds)
      for (const axis of ['x', 'y', 'z', 'e'] as const) {
        r.maxAxisSpeed[axis] = Math.max(
          r.maxAxisSpeed[axis],
          move.axisSpeeds[axis],
        );
    ...
    if (
      profile?.maxExtrusionSpeed !== undefined &&
      speed > profile.maxExtrusionSpeed
    )
      this.violation(
    ...
    if (profile?.filamentDiameter && extruding) {
      ...
      if (
        profile.maxVolumetricFlow !== undefined &&
        flow > profile.maxVolumetricFlow
      )
        this.violation(
