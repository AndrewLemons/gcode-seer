...
export class Interpreter {
	...
	private execute(command: Command): AnalysisEvent[] {
		...
		if (code === "M400") {
			...
			if (has("U")) {
				this.suspended = true;
				return unsupported("Bambu M400 U requires user interaction. Interpretation stops here.");
			}
		...
		if (["G20", "G21", "G90", "G91", "M82", "M83", "G17"].includes(code)) {
			...
			if (code === "G20" || code === "G21") {
				...
				this.extruderAbsolute = code === "M82";
			}
			return events;
		}
		...
		if (code === "G28") {
			...
			for (const axis of selected.length ? selected : axes) {
				this.position[axis] = this.options.printer?.homePosition?.[axis] ?? null;
				this.offset[axis] = 0;
			}
		...
		if (code === "G92") {
			...
			for (const axis of axes) {
				if (has(axis.toUpperCase()) || resetAll) {
					...
					this.offset[axis] =
						this.position[axis] === null ? null : this.position[axis] - coordinate;
			...
			if (has("E") || resetAll) {
				...
				this.extrusionRegisters.set(
					this.register(),
					(value("E") ?? 0) * (this.volumetric.has(this.tool) ? this.unit ** 3 : this.unit),
				);
			}
			return events;
		}
		...
		if (code === "M200") {
			...
			if (diameter === 0) {
				this.volumetric.delete(target);
			} else {
				this.volumetric.set(target, Math.PI * ((diameter * this.unit) / 2) ** 2);
			}
			return events;
		}
		if (code === "M149") {
		...
		if (has("E")) {
			if (this.tool === null) {
				...
				const requested = value("E")! * (area === undefined ? this.unit : this.unit ** 3);
				const mode =
				...
				this.extrusionRegisters.set(
					register,
					mode === null ? null : mode ? requested : previous === null ? null : previous + requested,
				);
				const flow = this.flowFactors.get(this.tool) ?? (this.uncertainTools ? null : 1);
				extrusion =
					delta === null ||
					flow === null ||
		...
		const feedrate =
			this.feedrate === null || this.speedFactor === null ? null : this.feedrate * this.speedFactor;
