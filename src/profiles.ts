...
function validateDialect(dialect: Dialect): void {
  if (
    !dialect.name ||
    !['marlin', 'independent', 'relative-override'].includes(
      dialect.extrusionMode,
    ) ||
    ...
    !dialect.passiveCommands.every(
      (command) =>
        typeof command === 'string' && command === command.toUpperCase(),
    )
  )
...
const nonnegative = (value: number, name: string): void => {
  finite(value, name);
  if (value < 0) throw new TypeError(`${name} cannot be negative.`);
};
...
export function validateOptions(options: AnalyzeOptions): void {
  if (options.dialect) validateDialect(options.dialect);
  ...
  if (options.printer) validatePrinterProfile(options.printer);
}
export function validatePrinterProfile(profile: PrinterProfile): void {
  ...
  if (profile.dialect) validateDialect(profile.dialect);
  if (profile.travelBounds) box(profile.travelBounds, 'travelBounds');
  if (profile.printBounds) box(profile.printBounds, 'printBounds');
  ...
  for (const zone of profile.exclusions ?? []) {
    ...
    if (zone.minZ !== undefined) finite(zone.minZ, 'minZ');
    if (zone.maxZ !== undefined) finite(zone.maxZ, 'maxZ');
  ...
  for (const [id, heater] of Object.entries(profile.heaterTargets ?? {})) {
    if (
      !/^\d+$/.test(id) ||
      Number(id) > 255 ||
      !profile.tools?.some((t) => t.heater === heater)
    )
      throw new TypeError(
        'Heater selectors must map IDs 0 to 255 to configured tool heaters.',
      );
  }
  const tools = new Set<number>();
  for (const tool of profile.tools ?? []) {
    if (
      !Number.isInteger(tool.id) ||
      tool.id < 0 ||
      tool.id > 255 ||
      tools.has(tool.id)
    )
      throw new TypeError(
        'Tool IDs must be unique integers between 0 and 255.',
      );
    if (!tool.heater || ['bed', 'chamber'].includes(tool.heater))
      throw new TypeError(
        'Tool heaters must have nonempty names other than bed or chamber.',
      );
    tools.add(tool.id);
    if (tool.travelBounds) box(tool.travelBounds, 'tool.travelBounds');
    if (tool.printBounds) box(tool.printBounds, 'tool.printBounds');
    for (const [name, value] of Object.entries(tool))
      if (typeof value === 'number') nonnegative(value, `tool.${name}`);
    if (tool.filamentDiameter === 0)
      throw new TypeError('Filament diameter must be positive.');
    if (
      tool.maxVolumetricFlow !== undefined &&
      tool.filamentDiameter === undefined
    )
    ...
    if (
      ...
      throw new TypeError(
        'Minimum extrusion temperature exceeds the heater maximum.',
      );
  }
}
...
export function bambuExclusion(
  ...
  const polygon = points.map((point) => {
    const match =
      /^\s*([+-]?(?:\d+(?:\.\d*)?|\.\d+))x([+-]?(?:\d+(?:\.\d*)?|\.\d+))\s*$/.exec(
        point,
      );
    if (!match) throw new TypeError(`Invalid Bambu exclusion point: ${point}`);
