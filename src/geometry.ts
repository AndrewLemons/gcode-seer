...
export function arcParameter(path: ArcPath, angle: number): number | null {
  const travel =
    path.sweep > 0
      ? normalized(angle - path.startAngle)
      : normalized(path.startAngle - angle);
...
export function pathBounds(path: MotionPath): Box {
  ...
  if (path.kind === 'arc') {
    for (const angle of [0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2]) {
      ...
      if (t !== null) expandBox(box, pointAt(path, t));
    }
  }
  return box;
}
...
export function containsBox(outer: Box, inner: Box): boolean {
  return axes.every(
    (axis) =>
      inner.min[axis] >= outer.min[axis] - EPSILON &&
      inner.max[axis] <= outer.max[axis] + EPSILON,
  );
}
export function pointInPolygon(
  ...
  for (let i = 0; i < polygon.length; i++) {
    ...
    if (
      Math.abs(cross(ab, ap)) <=
        EPSILON * Math.max(1, Math.hypot(ab.x, ab.y)) &&
...
export function intersectsExclusion(
  ...
  if (dz !== 0) {
    if (zone.minZ !== undefined) add((zone.minZ - path.start.z) / dz);
    if (zone.maxZ !== undefined) add((zone.maxZ - path.start.z) / dz);
  }
    ...
    if (path.kind === 'line') {
      ...
      if (Math.abs(denominator) > EPSILON) {
        ...
        if (length2 > 0)
          for (const p of [a, b])
            add(
              ((p.x - path.start.x) * direction.x +
                (p.y - path.start.y) * direction.y) /
                length2,
            );
      }
    } else {
      ...
      if (discriminant < -EPSILON || aa === 0) continue;
      for (const u of [
        ...
        const t = arcParameter(
          path,
          Math.atan2(
            a.y + u * edge.y - path.center.y,
            a.x + u * edge.x - path.center.x,
          ),
        );
        if (t !== null) add(t);
      }
  ...
  times.sort((a, b) => a - b);
  return times.some(
    (t, i) => contained(t) || (i > 0 && contained((t + times[i - 1]!) / 2)),
  );
}
...
export function createArc(
  ...
  if (radius <= EPSILON) throw new Error('Arc radius must be positive.');
