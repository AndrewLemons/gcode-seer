...
describe('continuous geometry', () => {
  it('detects a crossed exclusion when both endpoints are clear', () => {
    ...
    expect(analyze('G1 X10 F600', { ...base, printer }).constraints).toBe(
      'violated',
    );
  });
  ...
  it('finds circular extrema that are absent from endpoints', () => {
    ...
    expect(report.maxAxisSpeed.x).toBeCloseTo(
      (10 * Math.PI) / report.nominalDuration,
    );
  });
  it('detects narrow exclusions along the arc instead of testing the chord', () => {
    ...
    expect(
      intersectsExclusion(
        { kind: 'line', start: arc.start, end: arc.end },
        narrow,
      ),
    ).toBe(false);
  });
  it('distinguishes short and long signed-radius arcs', () => {
    ...
    expect(createArc(start, end, false, { r: 1 }).sweep).toBeCloseTo(
      Math.PI / 2,
    );
    expect(createArc(start, end, false, { r: -1 }).sweep).toBeCloseTo(
      (3 * Math.PI) / 2,
    );
    expect(createArc(start, end, true, { r: 1 }).sweep).toBeCloseTo(
      -Math.PI / 2,
    );
  });
  ...
  it('rejects impossible or ambiguous arcs', () => {
    ...
    expect(() =>
      createArc(origin, { x: 10, y: 0, z: 0 }, false, { r: 1 }),
    ).toThrow();
    expect(() =>
      createArc(origin, { x: 1, y: 0, z: 0 }, false, { i: 1, r: 1 }),
    ).toThrow();
