...
import { analyze, bambuDialect, klipperDialect, marlinDialect } from "../src/index.js";
import type { AnalyzeOptions, Dialect, MoveEvent, PrinterProfile } from "../src/index.js";
...
describe("motion state", () => {
	...
	it("resolves relative XYZ, absolute XYZ, G92 physical origins and unit changes", () => {
		const report = analyze("G1 X10 Y20 Z1 F600\nG92 X0\nG1 X5\nG91\nG1 X2 Z3\nG90\nG1 X1", base);
	...
	it("applies M220 to modal feedrate and M221 to extrusion deltas", () => {
		const report = analyze("M221 S150\nG1 X10 E2 F600\nM220 S50\nG1 X20 E4", base);
	...
	it("uses Marlin mode overrides that clear on G90/G91", () => {
		expect(analyze("M83\nG1 E2 F60\nG90\nG1 E3", base).tools[0]?.extruded).toBe(3);
		expect(analyze("G91\nM82\nG1 E2 F60\nG1 E3", base).tools[0]?.extruded).toBe(3);
	});
	...
	it.each([klipperDialect, bambuDialect])(
		...
		(dialect) => {
			expect(analyze("M83\nG1 E2 F60\nG90\nG1 E3", { ...base, dialect }).tools[0]?.extruded).toBe(
				5,
			);
			expect(analyze("G91\nM82\nG1 E2 F60\nG1 E3", { ...base, dialect }).tools[0]?.extruded).toBe(
				5,
	...
	it("rejects inches for Klipper", () => {
		expect(analyze("G20", { dialect: klipperDialect }).validity).toBe("invalid");
	});
	...
	it.each(["M201 X1", "M203 X1", "M204 P1", "M205 X1"])(
		"reports %s without applying planner limits to requested motion",
		(command) => {
			const report = analyze(`${command}\nG1 X10 F600`, base);
			expect(report.complete).toBe(true);
			expect(report.maxAxisSpeed.x).toBe(10);
			expect(report.nominalDuration).toBe(1);
			expect(report.diagnostics).toContainEqual(
				expect.objectContaining({ code: "PLANNER_SETTINGS", severity: "info" }),
			);
		},
	);
...
describe("temperatures and tools", () => {
	...
	it("supports shared and independent tool E registers", () => {
		...
		expect(analyze(program, { ...base, dialect: logical }).tools[0]?.extruded).toBe(19);
		expect(
			analyze(program, {
				...base,
	...
	it("models Bambu wait durations and stops at interactive pauses", () => {
		expect(analyze("M400 S2 P500\nG4 P500", { dialect: bambuDialect }).nominalDuration).toBe(3);
		expect(analyze("M400 U1\nG1 X10 F600", { ...base, dialect: bambuDialect }).moves).toBe(0);
	});
});

describe("coverage and extensibility", () => {
	it.each(["M104", "M109", "M220", "M221", "M200", "G999\nM221 S100", "G999\nM200 D0"])(
