...
describe('profiles and constraints', () => {
  it('imports Bambu exclusions and returns independent profile instances', () => {
    expect(bambuExclusion(['0x0', '18x0', '18x28', '0x28']).polygon[2]).toEqual(
      { x: 18, y: 28 },
    );
    ...
    expect(
      analyze('M83\nG1 X10 Y10 E1 F600', { ...base, printer: b }).constraints,
    ).toBe('violated');
  });
  ...
  it('checks per-tool geometry', () => {
    ...
    expect(
      analyze('G1 X10 E1 F600', { ...base, printer }).diagnostics.map(
        (d) => d.code,
      ),
