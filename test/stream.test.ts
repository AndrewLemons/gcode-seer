...
describe('streaming and lifecycle', () => {
  it('does not share live interpreter position with event observers', () => {
    const report = analyze('G91\nG1 X10 F600\nG1 X10', {
      ...options,
      onEvent(event) {
        if (event.type === 'move') event.end.x = 999;
      },
    });
  ...
  it('handles one-byte chunks and a Node Readable', async () => {
    const chunks = Array.from(new TextEncoder().encode(text), (byte) =>
      Uint8Array.of(byte),
    );
    expect(await analyzeStream(Readable.from(chunks), options)).toEqual(
  ...
  it('supports mixed text and complete UTF-8 chunks', async () => {
    expect(
      (
        await analyzeStream(
          ['G1 ', new TextEncoder().encode('X10 F600\r'), '', '\n'],
          options,
        )
      ).lines,
    ).toBe(1);
  ...
  it('honors cancellation', async () => {
    ...
    await expect(
      analyzeStream(['G21'], { signal: controller.signal }),
    ).rejects.toThrow();
  });
