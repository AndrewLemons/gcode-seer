...
Releases use **Release Please → GitHub Releases → npm staging → maintainer approval with 2FA**. Versioning, changelogs, tags, builds, and staging are automated. npm notifies maintainers when a staged package is ready. A maintainer reviews and approves each staged version before it becomes available on npm.
...
7. npm notifies maintainers about the staged package. The staging job's Actions summary records the stage ID, artifact integrity, and a link to review instructions.
8. The maintainer inspects the staged package and approves it on npm with 2FA. A green workflow means staging succeeded, an existing publication was verified, or the summary reported that first-publication bootstrap is required; it does **not** mean a new version was published.
...
3. Publish builds and verifies the package, detects the missing npm package, and reports that interactive bootstrap is required in the Actions summary. No registry write is attempted.
4. From that workflow run, download and inspect the **tested** `npm-package` artifact. With Node 24 and npm CLI 11.19.1 installed locally, log in as the intended owner and publish that tarball interactively:
   ...
   Replace `RUN_ID` with the ID from that workflow run's URL. Confirm name ownership, inspect the archive, and compare its SHA-512 integrity with the Actions summary before publishing. Use your own npm login with 2FA and remove CI-token environment variables from your local shell. Do not rebuild a different tarball.
...
6. Rerun Publish on that release's tag to verify the published version's integrity.
...
npm offers a **Staged Packages** tab on npmjs.com. Review the matching package, version, and stage ID there and choose **Approve**; npm prompts for 2FA. Alternatively, use these commands with your personal npm session:
...
Use npm CLI 11.15+ (the workflow pins 11.19.1). Replace `STAGE_ID` with the UUID from the staging job's Actions summary, never an unreviewed stage from another run. To compute the downloaded file's integrity using Bun:
...
Compare that value with the Actions summary. Approval publishes the staged bytes. To discard a stage, run `npm stage reject STAGE_ID` and complete 2FA. Staging does not reserve a separately publishable copy: npm uses the same version uniqueness constraint for staged and published versions. The workflow never rejects or replaces a stage automatically. See [npm stage command behavior](https://docs.npmjs.com/cli/v11/commands/npm-stage/).

Stages target `latest`, and npm fixes the tag when staging. Approve pending versions in ascending order so an older version does not become the default install. After approval, rerun Publish on its tag if you want automatic public-integrity verification.

## Notifications

npm handles staged-package notifications. The staging job's Actions summary provides the stage ID, tested artifact integrity, and release instructions. For a missing package, check that summary for the first-publication bootstrap requirement; there is no npm stage to notify about yet.
...
- **Missing package:** follow the First publication steps linked from the Actions summary; CI does not fall back to direct publication.
